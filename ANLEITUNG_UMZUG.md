...
├── img/                ← aus dist/  (App-Icon + Trail-Abzeichen, jetzt lokal)
├── manifest.json       ← aus dist/
└── .htaccess           ← aus "htaccess_fuer_webspace.txt", umbenannt zu .htaccess
```

Hinweis: Das App-Icon und die Trail-Abzeichen sind jetzt **fest in die App
eingebaut** (Ordner `img/`) und werden nicht mehr von hs-bw.com geladen. Damit
ist das Problem mit dem fehlenden Login-Logo behoben.
