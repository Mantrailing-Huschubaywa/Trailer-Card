...
const App: React.FC = () => {
  ...
  const mapProfileRoleToUserRole = (role: string): UserRoleEnum => {
    if (role === 'admin') return UserRoleEnum.ADMIN;
    if (role === 'staff') return UserRoleEnum.MITARBEITER;
    return UserRoleEnum.KUNDE;
  };

  const buildUserFromProfile = (profile: { id: string; email: string | null; display_name: string | null; role: string; customer_id: string | null; }): User => {
    const name = (profile.display_name || '').trim();
    const parts = name.split(' ').filter(Boolean);
    const firstName = parts[0] || (profile.email ? profile.email.split('@')[0] : '');
    ...
    return {
      ...
      createdAt: REFERENCE_DATE.toLocaleDateString('de-DE'),
      associatedCustomerId: profile.customer_id || undefined,
    };
  };

  const loadCurrentUserFromSupabase = async (userId: string) => {
    const { data, error } = await supabase
      .from('profiles')
      .select('id,email,display_name,role,customer_id')
      .eq('id', userId)
      .single();
  ...
  useEffect(() => {
    // Initialize session on app load and keep in sync with auth state changes
    const init = async () => {
      ...
      if (session?.user) {
        setIsLoggedIn(true);
        setLoggedInUserEmail(session.user.email ?? null);
        await loadCurrentUserFromSupabase(session.user.id);
      } else {
        setIsLoggedIn(false);
  ...
  const handleLogin = async (email: string, passwordInput: string): Promise<boolean> => {
    if (!hasSupabaseEnv) {
      alert('Supabase ist nicht konfiguriert. Bitte VITE_SUPABASE_URL und VITE_SUPABASE_ANON_KEY setzen.');
      return false;
    }
    ...
    setIsLoggedIn(true);
    setLoggedInUserEmail(data.session.user.email ?? null);
    await loadCurrentUserFromSupabase(data.session.user.id);

    // Redirect happens in the role-based useEffect, so we just return success.
    return true;
  };
