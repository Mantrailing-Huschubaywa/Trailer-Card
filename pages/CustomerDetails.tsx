...
// Anzeige der Seminar/Event-Abzeichen (aus der ursprünglichen Version wiederhergestellt)
const SeminarEventPatches: React.FC<{ totalSeminars: number }> = ({ totalSeminars }) => {
  ...
  const rows = [];
  const rowSize = 4; // Maximal 4 Abzeichen pro Reihe
    ...
    for (let j = i; j < i + rowSize && j < totalSeminars; j++) {
      patchGroup.push(
        <div key={`seminar-${j}`} className="relative first:ml-0 -ml-12">
          <SeminarEventPatchIcon className="h-32 w-32 [filter:drop-shadow(0_2px_2px_rgba(0,0,0,0.25))]" />
        </div>
      );
    }
    rows.push(
...
const SetInitialValuesModal: React.FC<SetInitialValuesModalProps> = ({ isOpen, onClose, onSubmit, dogs, currentSeminars, isSubmitting = false }) => {
...
const CustomerDetails: React.FC<CustomerDetailsProps> = ({
  ...
  const handleSetInitialValues = async (dogTrails: Record<string, number>, newTotalSeminars: number) => {
    ...
    // --- SEMINARE/EVENTS: Bestandsübernahme anpassen ---
    // Reguläre Seminar-Buchungen bleiben IMMER unberührt; angepasst wird nur
    // die Zahl der "Bestandsübernahme"-Platzhalter, um auf die Gesamtzahl zu kommen.
    const regularSeminars = customerTransactions.filter(
      t => t.description === 'Workshop' || t.description === 'Seminar/Event'
    );
    if (newTotalSeminars < regularSeminars.length) {
      alert(`Korrektur nicht möglich: Die eingegebene Gesamtzahl (${newTotalSeminars}) ist geringer als die Anzahl der bereits regulär gebuchten Seminare (${regularSeminars.length}).`);
      setIsSubmittingTransaction(false);
      return;
    }
    const bestandSeminars = customerTransactions.filter(t => t.description === 'Workshop (Bestandsübernahme)');
    const seminarDiff = (newTotalSeminars - regularSeminars.length) - bestandSeminars.length;
    let seminarIdsToDelete: string[] = [];
    if (seminarDiff > 0) {
      for (let i = 0; i < seminarDiff; i++) {
        trailTransactionsToInsert.push({
          id: `trx-seminar-${crypto.randomUUID()}`,
          customerId: customer.id,
          type: 'debit',
    ...
    } else if (seminarDiff < 0) {
      seminarIdsToDelete = bestandSeminars.slice(0, Math.abs(seminarDiff)).map(t => t.id);
    }
    ...
    const newTotalTransactions = customer.totalTransactions + trailTransactionsToInsert.length - seminarIdsToDelete.length;
    ...
    if (seminarIdsToDelete.length > 0) {
      await onDeleteTransactionsByIds(seminarIdsToDelete);
    }
  ...
  // Seminare/Events werden aus den vorhandenen Transaktionen gezählt (nichts wird verändert)
